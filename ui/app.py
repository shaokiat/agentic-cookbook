...
# Two levels: a handful of top-level expanders (collapsible) containing bolded
# subgroup headers (not collapsible — Streamlit doesn't allow nested expanders).
# Subgroup names mirror the Level 1-7 concept ladder in PROJECT_CONTEXT.md.
nav_pages = {
    "Core Agent Loop": {
        "Primitives": [
...
page_list = [p for subgroups in nav_pages.values() for pages in subgroups.values() for p in pages]
nav = st.navigation(page_list, position="hidden")

st.sidebar.title("🍳 agentic-cookbook")
model_picker()

for group, subgroups in nav_pages.items():
    group_pages = [p for pages in subgroups.values() for p in pages]
    with st.sidebar.expander(group, expanded=(nav in group_pages)):
        for subgroup, pages in subgroups.items():
            st.markdown(f"**{subgroup}**")
            for p in pages:
                st.page_link(p, label=p.title)

n_chapters = sum(len(subgroups) for subgroups in nav_pages.values())
st.sidebar.caption(f"{len(page_list)} examples across {n_chapters} chapters")
nav.run()
